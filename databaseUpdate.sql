-- Criar o banco de dados eeepma26_biblioteca se não existir
CREATE DATABASE IF NOT EXISTS eeepma26_biblioteca;
USE eeepma26_biblioteca;  -- Selecionar o banco de dados eeepma26_biblioteca
...
CREATE TABLE IF NOT EXISTS aluno (
  ...
  serie VARCHAR(1),
  PRIMARY KEY (id),
  FOREIGN KEY (sala_identificacao, curso) REFERENCES turma(nome_identificacao, curso)  -- Chave estrangeira para turma
);
...
CREATE TABLE IF NOT EXISTS emprestimos (
  ...
  PRIMARY KEY (id),
  FOREIGN KEY (aluno_id) REFERENCES aluno(id),  -- Chave estrangeira para aluno
  FOREIGN KEY (titulo_livro, numero_registro) REFERENCES livros(titulo_livro, numero_registro)  -- Chave estrangeira para livros
);
