...
<body>
    ...
    if (query.length < 3) {
        // Não faz a busca se o termo for muito curto
        document.getElementById('search-results').innerHTML = '<p>Digite ao menos 3 caracteres para buscar.</p>';
        return;
    }

    // Realiza a requisição para search.php
    fetch(`search.php?q=${encodeURIComponent(query)}`)
        ...
        .catch(error => {
            ...
            document.getElementById('search-results').innerHTML = '<p>Erro ao buscar livros. Tente novamente.</p>';
        });
});
